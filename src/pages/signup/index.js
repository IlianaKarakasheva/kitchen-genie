...
useEffect(()=>{
  ...
  return (
    <div className='signUpPage'>
      ...
      <input type={"email"} placeholder='E-mail address' name='emailreg'value={userData.email} onChange={(event) =>setUserData({...userData, email:event.target.value})} class="form-control"/>
      {errors.emailreg && <span className='error text-danger'> {errors.emailreg}</span>}
      <input type={'password'} placeholder='Password' name='passreg' value={userData.password} onChange={(event) => setUserData({...userData, password:event.target.value})} class="form-control"/>
      {errors.passreg && <span className='error text-danger'> {errors.passreg}</span>}
      <button className='signUpButton btn btn-lg btn-primary btn-block' onClick={handleRegister}>SIGN UP</button>
      <h3>
        User logged in:
      </h3>
      {user?.email}
